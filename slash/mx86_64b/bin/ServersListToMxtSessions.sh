...
/bin/MobaBox.exe MobaSendMsg MobaXterm "-selectfile|Please select your list of servers|$$.tmp"
...
if [ -e "/tmp/$$.tmp" ]; then
  ...
  if [ -e "$MyFile" ]; then
    MyServerList="`toybox convpath -u "$MyFile"`"
    echo
    ...
    echo
    echo
    echo "Your sessions have been saved to file"
    toybox convpath -w "$TmpFile"
    echo
